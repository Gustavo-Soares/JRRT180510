...
public class MoveMethodTests extends TestCase {
	...
	public void testFail2(String tp_name, String sig, Program in) {		
		assertNotNull(in);
		TypeDecl tp = in.findType(tp_name);
		...
		try {
			...
			md.doMoveToField("f", false, true, true);
			fail("Refactoring was supposed to fail; succeeded with "+in);
		} catch(RefactoringException rfe) { System.out.println(rfe.getMessage());}
	}
